#!/usr/bin/env node
// Generates cross-SDK test vectors for the Swift client to verify against.
//
// Output:
//   ../Tests/XyoClientTests/Resources/jsCompatVectors.json
//   ../Tests/XyoChainProtocolTests/Resources/jsCompatVectors.json
//
// The vector file is a frozen contract between the JS and Swift SDKs at
// specific pinned versions (see package.json). Regenerating it is a deliberate
// act; when output diverges, the Swift side must be re-verified.
//
// This is a faithful port of the Android SDK's xl1-compat/generate-vectors.mjs,
// with the output paths pointed at the Swift test resource bundles and a
// `root_hash` field added to each BoundWitness vector (the full hash including
// `$signatures`).
...
const outputPaths = [
  resolve(__dirname, '../Tests/XyoClientTests/Resources/jsCompatVectors.json'),
  resolve(__dirname, '../Tests/XyoChainProtocolTests/Resources/jsCompatVectors.json'),
]
...
// Each raw payload is emitted as JSON.stringify output so the Swift side
// has a concrete string to feed into JSONSerialization + canonicalization. The
...
const payloadCases = [
  ...
  // also drops them — the wire JSON Swift receives has no representation for
  ...
  // Plugin payload-type vectors — exercise each Swift Payload/Core/* class
  // end-to-end. Swift constructs the equivalent type and dataHash()
...
// the canonical dataHash, each signer's signature, and the root hash (the
// full hash including $signatures).
...
async function buildBoundWitnessVectors() {
  ...
  for (const { id, signers: signerKeys, payloads } of boundWitnessCases) {
    ...
    // Root hash: the full canonical hash including $signatures (only _ meta stripped).
    const rootHash = await ObjectHasher.hash(signed)
    vectors.push({
      id,
...
// 10^k atto. The Swift side uses BigInt; JS uses bigint. Vectors are
